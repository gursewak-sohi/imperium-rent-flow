<template>
   <div class = "manage_cases custom_manage_cases">
      ...
      <CCard>
         ...
         <CCardHeader>
            <slot name="header">
               <CRow class="align-items-center" >
                  <CCol  v-if="user_auth_role == 'super_admin'"> 
                     <div role="group " class="form-group" >
                              <label for="role_group" > Organisation </label>
                              <select id="role_group" class="form-control" v-model="selected_organisation"  @change="updateOrgEmployee">
                  ...
                  <CCol > 
                     <div role="group" class="form-group button_grp">
                     <CButton  color="primary"  @click="filterMyResult()">                  
                           <CIcon name="cil-plus"/>
                        Filter
                     </CButton>
                     <!-- <CButton  color="success"  @click="roleModal = 'add';showRoleModal=true;">                   -->
                     <CButton  color="success"  @click="CreateCaseInNewWindow()">                  
                           <CIcon name="cil-plus"/>
                        Create Case
                     </CButton>
                     </div>
         ...
         <div class="assigned_panel d-flex col mt-2 justify-content-end">
         <div class="mr-3">Un Assigned Cases : <a class="handPointer" v-on:click="goTo('unassigned_cases')"><b><u>  {{ total_unassigned_cases }} </u> </b></a></div>
         <div>Deadup Cases : <a class="handPointer"><b v-on:click="goTo('dead-cases')"> <u>{{ dead_cases }} </u> </b></a>
   </div>
         </div>
         ...
         <CCardBody class="pt-2">
            <CDataTable
               ...
               striped
               fixed
               :items="this.employee_case_list"
               :fields="['agent', 'total', 'in_progress', 'out_of_tat', 'positive_verified', 'negative_verified','total_closed', 'first_time', 'last_time', 'location']"
               :items-per-page="20"
               pagination
               color="dark"
               class="table-bordered custom_manage_cases_table"
               >

               <template #agent="{item}">
                  <td>
                    <a class="handPointer" v-on:click="gotoEmployeeView(item)"> {{ item.agent ? item.agent:'N/A'}} </a>
                  </td>
               </template>
               <template #total="{item}">
                  <td>
                    <a class="handPointer" v-on:click="gotoEmployeeView(item)"> {{ item.total ? item.total:'0'}} </a>
                  </td>
               </template>
               <template #in_progress="{item}">
                  <td>
                    <a class="handPointer" v-on:click="gotoEmployeeView(item)"> {{ item.in_progress ? item.in_progress:'0'}} </a>
                  </td>
               </template>
               <template #out_of_tat="{item}">
                  <td>
                    <a class="handPointer" v-on:click="gotoEmployeeView(item)"> {{ item.out_of_tat ? item.out_of_tat:'0'}} </a>
                  </td>
               </template>
               <template #positive_verified="{item}">
                  <td>
                    <a class="handPointer" v-on:click="gotoEmployeeView(item)"> {{ item.positive_verified ? item.positive_verified:'0'}} </a>
                  </td>
               </template>
               <template #negative_verified="{item}">
                  <td>
                    <a class="handPointer" v-on:click="gotoEmployeeView(item)"> {{ item.negative_verified ? item.negative_verified:'0'}} </a>
                  </td>
               </template>
               <template #total_closed="{item}">
                  <td>
                    <a class="handPointer" v-on:click="gotoEmployeeView(item)"> {{ item.total_closed ? item.total_closed:'0'}} </a>
                  </td>
               </template>
               <template #first_time="{item}">
                  <td>
                    <a class="handPointer" v-on:click="gotoEmployeeView(item)"> {{ item.first_time ? item.first_time:'N/A'}} </a>
                  </td>
               </template>
               <template #last_time="{item}">
                  <td>
                    <a class="handPointer" v-on:click="gotoEmployeeView(item)"> {{ item.last_time ? item.last_time:'N/A'}} </a>
                  </td>
               </template>

               <template #location="{item}">
                  <td>
                     <template v-if="item.agent != 'Total'">
                        <img class="handPointer" :src="'https://severify.com/assets/images/icon/admin/map.png'" style="width: 30px;" v-on:click="showPointers(item)" v-c-tooltip.hover.click="'View Map'"/> 
      ...
      <CaseLocationsModal  @closeModal="closeChildModel" v-if="showMapModal" :location="location"/>
   </div>
</template>
<script>
   ...
   import CaseLocationsModal from './CaseLocationsModal.vue';
     ...
     methods: {
         ...
         //   formData.append('end_date', this.ed);
            ...
            axios.post('https://severify.com/api/employee/all-cases', formData)
               ...
               vm.dead_cases = response.data.dead_cases;
                if(vm.user_auth_role != 'super_admin'){
                  let company_id =localStorage.getItem('company_id');
                  vm.selected_organisation_for_case = company_id;
                  vm.selected_organisation = company_id;

                  console.log(vm.selected_organisation_for_case , 'Case Id');
                  vm.updateEmployee();
               } 
        ...
        showPointers(rowDetails){
         ...
         changeCaseType(type){
          ...
          CreateCaseInNewWindow(){
            //  window.open("admin/create-case");
            //  alert(12);
             let routeData = this.$router.resolve({name: 'Create Case', query: {data: "someData"}});
